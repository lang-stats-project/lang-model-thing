...
# =============================================================== #
# --------------------------------------------------------------- #
# set if you want to search for the .conf files for the label set (recommended)
SEARCH_CONF_FILES=false 
# --------------------------------------------------------------- #
...
NEW_BOF_FILES=true 
# --------------------------------------------------------------- #
# if you want to run the experiment on the rocks queue for the evaluation pipeline
...
# (only use leading slashes '/' for abs path)
# =============================================================== #
# This is of course assuming you've generated .bof files for the dataset...
#
EXPERIMENT_NAME=$NUM_TOPICS"_topic_model_real_fake"
WORKING_DIR="/home/nwolfe/EAGEAE_krsvm"
KERNEL="X2"
#KERNEL="LINEAR"
DCMETHOD="SSE"
#DCMETHOD="Sparse"
CLASSIFIER="SVM"
#CLASSIFIER="SVM,KR"
DATASET="MED14-MEDTEST-EK100" 
#DATASET="MED14-MEDTEST-EK10" 
...
cd $HOME_DIRECTORY
# =============================================================== #
# GET CONFIDENCE FILES FOR DATASET
# =============================================================== #
if [ "$SEARCH_CONF_FILES" = true ]; then

	echo "Looking up confidence files..."
	./search_conf_files.py $CONF_FILES_DIRECTORY $LOCAL_CONF_DIRECTORY $CONF_ID_FILE_MAPPING $TEST_IDX_FILE $LABELSET
...
# =============================================================== #
# GENERATE CONFIG FILE AND RUN ON ROCKS EVALUATION PIPELINE...
# =============================================================== #
if [ "$SUBMIT_FOR_EVAL" = true ] ; then

	echo "Generating .config file..."
	./gen_config_alt.py $CONFIG_FILE $EXPERIMENT_NAME $BOF_LIST_FILE $DATASET $WORKING_DIR $KERNEL $DCMETHOD $CLASSIFIER

	echo "Running experiment on rocks..."
	if [ -f $WORKING_DIR"/"$CONFIG_FILE ] ; then

		rm $WORKING_DIR"/"$CONFIG_FILE	
	...
	# ================================= #
	echo "y" | perl eageae_wrapper.pl $CONFIG_FILE
	# ================================= #
fi
# =============================================================== #
# FINISH
