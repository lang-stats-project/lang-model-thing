...
bof_file = open(list_file).readlines()[0].strip()
data = [int(d.split(":")[0]) for d in open(bof_file).readlines()[0].strip().split(" ")]
dimensions = max(data)
...
config_name.write("kernel=X2" + "\n")
config_name.write("dimensions=" + str(dimensions) + "\n")
config_name.write("dcmethod=SSE" + "\n")
config_name.write("dctimehours=24" + "\n")
config_name.write("classifier=SVM,KR" + "\n")
